...
def print_agent_step(agent_name, message, color_code="36"):
    ...
    print(f"\033[1;{color_code}m[{timestamp}] [{agent_name.upper():<16}] {message}\033[0m")
...
def mock_llm_entity_extraction(text_content):
    ...
    if "invoice" in text_lower:
        ...
        entities["error"] = "Could not determine document structure for entity extraction."

    print_agent_step("LLM Service", f"Extraction complete. Found {len(entities)-1} entities.", "35")
    return entities
        ...
        for filename in os.listdir(self.watch_directory):
            ...
            if os.path.isfile(file_path):
                ...
                try:
                    ...
                    print_agent_step("Ingestor Agent", f"File size: {file_size} bytes. Event 'INGESTED' created.")
                    return event
                except OSError as e:
                    print_agent_step("Ingestor Agent", f"Error accessing file {filename}: {e}", "31")
        ...
        try:
            ...
            if file_path.endswith('.txt'):
                with open(file_path, 'r') as f:
                    text_content = f.read()
                print_agent_step("Extractor Agent", "Text content successfully read from file.")
            else:
...
class ClassifierAgent:
    ...
    def run(self, event):
        ...
        print_agent_step("Classifier Agent", f"Received event for '{event['filename']}'")
        ...
        print_agent_step("Classifier Agent", f"Document classified as '{doc_type}' with {confidence:.2f} confidence.")
        return event
...
class RouterAgent:
    ...
    def run(self, event):
        ...
        if doc_type == "Invoice":
            routing_action = f"Calling ERP API with invoice data: {event['extracted_entities']}"
        ...
        print_agent_step("Router Agent", "Event 'ROUTED' created. Document processing complete.")
        return event
        ...
        while True:
            ...
            if event:
                ...
                try:
                    ...
                    print_agent_step("Orchestrator", f"Moved '{event['filename']}' to '{PROCESSED_DOCS_DIR}'", "34")
                except OSError as e:
